...
const SearchUser = ({ onClose }) => {
    ...
    const handleSearchUser = async (propsState) => {
        setLoading(true)
        try {
            const payload = { search };
            const actionName = ENDPOINTS.SEARCH_USER_LIST_ACTION;
    ...
    return (
        <div className='fixed top-0 bottom-0 left-0 right-0 bg-slate-700 bg-opacity-40 p-2 z-10'>
            <div className='w-full max-w-lg mx-auto mt-10'>
                ...
                {/**display search user */}
                <div className='bg-white mt-2 w-full p-4 rounded max-h-[500px] overflow-y-auto'>
                    {/**no user found */}
                    {
